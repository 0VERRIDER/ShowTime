...
//                    }
    ...
    public void loadPage(String pagename, MouseEvent event) throws IOException {
        Parent root = FXMLLoader.load(getClass().getResource("FXML/template2.fxml"));
    ...
    void bookTicket(ActionEvent event) throws IOException, ClassNotFoundException, SQLException {
        ...
        for (CheckBox checkbox : seats) {
            if (checkbox.isSelected()) {
                try {
                    n++;
                    q1 = "INSERT INTO seats (idTicket, rowId, columnId, idShow, idUser, is_available) VALUES(" + idTicket + "," + Character.getNumericValue(checkbox.getId().charAt(1)) + ",'" + checkbox.getId().charAt(0) + "'," + idShow + "," + CurrentUSerID + "," + 0 + ");";
                    connection = handler.getConnection();
                    pst = connection.prepareStatement(q1);
                    pst.executeUpdate();
                } catch (ClassNotFoundException ex) {
